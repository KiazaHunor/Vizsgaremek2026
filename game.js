...
function swapSlots(slot1, slot2) {
  ...
  updateScoreDisplay();
  // 🔥 EZT ADD HOZZÁ IDE
  document.querySelectorAll(".extra-player.locked").forEach(slot => {
    slot.innerHTML = renderDraftCard({
      name: slot.dataset.playerName,
      position: slot.dataset.position,
      team: slot.dataset.team,
      nationality: slot.dataset.nationality,
      rating: Number(slot.dataset.rating || 0)
    }, slot.dataset.imageSrc, 0);
...
function loadLeaderboard() {
  ...
  fetch("get_leaderboard.php?tournament_id=" + activeTournament.id)
    .then(r => r.json())
    .then(data => {
...
function startCountdown() {
  setInterval(() => {
    ...
    const end = new Date(activeTournament.entry_deadline).getTime();
    const now = new Date().getTime();

    const diff = end - now;

    if (diff <= 0) {
      document.getElementById("countdown").innerText = "Lezárva";
      return;
    }

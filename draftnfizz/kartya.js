...
if (!token) {  window.location.href = "../bejelentkezo/frontend/index.html";
}
...
function saveGameResult(result, playerScore, enemyScore) {
    ...
    .then(async response => {
        ...
        if (!text) {
            throw new Error("A PHP üres választ adott vissza.");
        }

        try {
            return JSON.parse(text);
        } catch (e) {
            throw new Error("A PHP nem JSON-t adott vissza: " + text);
        }
    })
    .then(data => {
